...
fn gelu_f32(x: f32) -> f32 {
    return 0.5f * x * (1.0f + Trig::stable_tanh(SQRT_2_OVER_PI * x * (1.0f + GELU_COEF_A * x * x)));
}
...
@compute @workgroup_size(64, 1, 1)
fn main_inplace(@builtin(global_invocation_id) invocation_id: vec3<u32>) {
    if (invocation_id.x < shape_src.nrows) {
        let isrc = Shape::iv(shape_src, invocation_id.x);
        src[isrc] = placeholder(src[isrc]);
    }
}
