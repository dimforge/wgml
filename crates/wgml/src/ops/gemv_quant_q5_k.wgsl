...
fn gemv(
    ...
    for (var j = 0u; j < shape_m.ncols; j++) {
        ...
        let parts0 = dequantize_q5_k_workgroup(quant0, local_id.x);
        let parts1 = dequantize_q5_k_workgroup(quant1, local_id.x);
        let parts2 = dequantize_q5_k_workgroup(quant2, local_id.x);
        let parts3 = dequantize_q5_k_workgroup(quant3, local_id.x);
...
// ################
fn dequantize_q5_k_workgroup(block_id: u32, k: u32) -> array<vec4<f32>, 2> {
