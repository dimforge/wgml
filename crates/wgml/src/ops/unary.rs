...
impl Unary {
    ...
    pub fn new(device: &Device, op: UnaryOp) -> Result<Self, ComposerError> {
        let mut composer = Shape::composer()?;
        WgTrig::compose(&mut composer)?;
...
/// Shader implementing various unary operations, applied in-place, selected with [`UnaryOp`].
pub struct UnaryInplace(pub ComputePipeline, pub UnaryOp);
...
impl UnaryInplace {
    ...
    pub fn new(device: &Device, op: UnaryOp) -> Result<Self, ComposerError> {
        ...
        Ok(Self(
            utils::load_module(device, "main_inplace", redirector.into_module().unwrap()),
            op,
        ))
    }
