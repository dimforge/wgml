...
impl GgufTensorData {
    ...
    pub fn to_gpu_matrix(
        &self,
        device: &Device,
        nrows: usize,
        ncols: usize,
    ) -> Option<GpuQuantMatrix> {
        // This flags is only for debugging purposes. Should be `false` for any real-world usage.
        // Set this to `true` to always dequantize the weights to f32 before sending
        // to the GPU.
        const ALWAYS_DEQUANTIZE: bool = false;

        macro_rules! match_quant {
            ($($match_id: ident, $gpu_ty: ident);*) => {
                match self {
                    $(Self::$match_id(v) => {
                        if v.len() % 2 == 0 && ncols % $gpu_ty::DEQUANTIZED_LEN == 0 {
                            // This can be interpreted as a BlockQ8_0x2 matrix.
                            let gpu_data: &[$gpu_ty] = bytemuck::cast_slice(v);
                            let mat =
                                DMatrix::from_column_slice(nrows, ncols / $gpu_ty::DEQUANTIZED_LEN, gpu_data);
                            return Some(GpuQuantMatrix::$match_id(GpuMatrix::init(
                                device,
                                &mat,
                                BufferUsages::STORAGE,
                            )));
                        } else {
                            info!("Failed to repack a quantized matrix. Dequantizing instead.");
                            // This won’t fit in a the quantized gpu matrix, dequantize.
                        }
                    })*
                    _ => { /* Other cases get de-quantized below. */ }
                }
            }
        }

        if !ALWAYS_DEQUANTIZE {
            match_quant!(
                Q8_0, GpuBlockQ8_0x2;
                Q5_0, GpuBlockQ5_0x2;
                Q5_1, GpuBlockQ5_1x2;
        ...
        self.dequantize().map(|deq| {
            let mat = DMatrix::from_column_slice(nrows, ncols, &deq);
            GpuQuantMatrix::F32(GpuMatrix::init(device, &mat, BufferUsages::STORAGE))
        })
    }
}
...
impl Gguf {
    ...
    pub fn print_metadata(&self) {
        for str in self.metadata_debug_strings() {
            println!("{}", str);
        }
    }

    pub fn metadata_debug_strings(&self) -> impl Iterator<Item = String> + '_ {
        self.metadata.iter().map(|(key, val)| {
            if let GgufMetadataValue::Array(arr) = val {
                format!("{} = array[{}]", key, arr.len())
            } else {
                format!("{} = {:?}", key, val)
            }
        })
    ...
    pub fn print_tensors(&self) {
        for str in self.tensors_debug_strings() {
            println!("{}", str);
        }
    }

    pub fn tensors_debug_strings(&self) -> impl Iterator<Item = String> + '_ {
        let mut keys: Vec<_> = self.tensors.keys().collect();
        keys.sort();

        keys.into_iter().map(|key| {
            let tensor = &self.tensors[key];
            format!("{} -> {:?}", key, tensor.dimensions())
        })
    }
}
